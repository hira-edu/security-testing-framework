...
ErrorHandler::ErrorHandler()
    : minimum_severity_(ErrorSeverity::INFO),
      log_outputs_mask_(LogOutput::CONSOLE | LogOutput::FILE),
...
void ErrorHandler::set_log_outputs(LogOutput outputs) {
    log_outputs_mask_.store(outputs);
...
void ErrorHandler::flush_logs() {
    ...
    for (auto& output : log_output_sinks_) {
        output->flush();
    }
}
...
void ErrorHandler::initialize_log_outputs() {
    ...
    LogOutput outputs = log_outputs_mask_.load();
    
    if (outputs & LogOutput::CONSOLE) {
        log_output_sinks_.push_back(std::make_unique<ConsoleLogOutput>());
    }
    
    if (outputs & LogOutput::FILE) {
        if (!log_file_path_.empty()) {
            log_output_sinks_.push_back(std::make_unique<FileLogOutput>(log_file_path_));
        }
    }
    
    if (outputs & LogOutput::EVENT_LOG) {
        log_output_sinks_.push_back(std::make_unique<EventLogOutput>());
    }
    
    if (outputs & LogOutput::DEBUGGER) {
        log_output_sinks_.push_back(std::make_unique<DebuggerLogOutput>());
    }
}
