...
PatternScanner::~PatternScanner() {
    ...
    auto cleanup_operation = PerformanceMonitor::GetInstance().start_operation("pattern_scanner_cleanup");
...
bool PatternScanner::Initialize() {
    ...
    auto init_operation = PerformanceMonitor::GetInstance().start_operation("pattern_scanner_initialization");
...
void PatternScanner::InitializeMemoryRegions() {
    ...
    auto region_operation = PerformanceMonitor::GetInstance().start_operation("memory_region_initialization");
    ...
    try {
        ...
        // End performance monitoring
        PerformanceMonitor::GetInstance().end_operation(region_operation);
...
std::vector<PatternScanResult> PatternScanner::ScanForPattern(
    ...
    auto scan_operation = PerformanceMonitor::GetInstance().start_operation("pattern_scanning");
    ...
    if (pattern.empty() || mask.empty() || pattern.size() != mask.size()) {
        ...
        PerformanceMonitor::GetInstance().end_operation(scan_operation);
        return results;
    }
